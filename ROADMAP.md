...
- Threshold crossing callouts: "fault fixed!" / "NEW fault" — **Python only. Removed from the app UI**, see the Beta section below
...
- Dart port with `SwingHistoryStore` and `SwingComparisonView` widget. **The Dart widget renders previous → current values only** — the trend and crossing machinery is computed and deliberately not shown
- **Key files**: `src/swing_history.py`, `flutter_app/lib/src/analysis/swing_history.dart`, `flutter_app/lib/src/analysis/swing_history_store.dart`, `flutter_app/lib/src/ui/widgets/swing_comparison_view.dart`
...
Goal of the beta is to test **retention** and to passively accumulate a corpus
of real swings — not to prove the detectors are right. The fault thresholds in
`faults.py` / `faults.dart` have never been validated against a labelled corpus,
so the app's presentation was pulled back to match what the data can support.
...
- **Fault presentation is tentative, not definitive.** A flagged fault renders as
  ...
  threshold labelled a "beta reference" rather than a verdict line. Detection,
  thresholds, and the drill recommendations are unchanged.

- **Cross-session judgment is not rendered.** The report's comparison card shows
  only raw previous → current measured values. The improved/worsened/unchanged
  trends, the FIXED / NEW FAULT badges, and the "you cleared X" / "the practice
  is paying off" callouts are not in the UI.

  Reason: with unvalidated thresholds, a threshold crossing between two swings
  may be measurement noise rather than a change in the golfer's swing. Telling a
  beta tester they improved when we cannot show it is the one claim most likely
  to cost trust.

  The `Trend` / `Crossing` machinery in `swing_history.dart` is **kept and stays
  unit-tested** — computed on every comparison and simply not read by the view.
  See the guardrail note on `SwingComparison.between`. Do not re-surface it in
  the UI until the thresholds are validated.
...
1. Enough beta swings collected to form a labelled corpus. Collection is now
   ...
   export.
2. Per-fault thresholds validated against that corpus, with a known
   false-positive rate. This is **P0.1 + P0.2** below — the same work, not a
   second effort.
...
Landed on `flutter-mvp`. Changes **what is recorded**, never what is measured:
...
- **Branch reconciliation is BLOCKED.** `main` carries `e86c8cf`, a revert that
  deleted `flutter_app/`, so merging `main` into `flutter-mvp` proposes deleting
  the entire Flutter app (19 silent deletions + 13 conflicts). It needs a
  deliberate resolution, not a routine merge. Until then `src/swing_history.py`
  and the P0.3 / GolfDB work remain `main`-only, and AUDIT.md item A1 stays open
  on this branch.
- **Off-device collection.** Manual export is the floor, not the answer;
  exit criterion 1 still needs a real path.
